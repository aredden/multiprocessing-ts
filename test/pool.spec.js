...
describe('Pool', function () {
	...
	describe('#map', function () {
		...
		it('should still work after processing multiple jobs', function () {
			...
			return P.all([pool.map(_.range(50), fn), pool.map(_.range(75), fn), pool.map(_.range(100), fn)])
				.then(function () {
		...
		it('should work with chunksize larger than job', function () {
			...
			return pool.map([5, 7, 1], fn, 100).then(function (result) {
		...
		it('should work with a module name', function () {
			const pool = new Pool(2);
			return pool.map(['Ryan', 'World'], `${__dirname}/sample-module`).then(function (result) {
				should.exist(result);
	...
	describe('#apply', function () {
		...
		it('should work with a module name', function () {
			const pool = new Pool(2);
			return pool.apply('World', `${__dirname}/sample-module`).then(function (result) {
				should.exist(result);
		...
		it('should be able to handle P based functions', function () {
			...
			const pool = new Pool(2);
			return P.all([pool.apply(4, fn), pool.apply(1, fn), pool.apply(3, fn), pool.apply(2, fn)]).then(
				function (results) {
	...
	describe('#define', function () {
		...
		it('should provide a simple wrapper over apply', function () {
			...
			return pool.square.apply(5).then(function (res) {
				res.should.equal(25);
			});
		});
		...
		it('should provide a simple wrapper over map', function () {
			const pool = new Pool(5);
			pool.define('square', (x) => x * x);
			return pool.square.map([5, 2, 3]).then(function (res) {
		...
		it('should work with modules', function () {
			const pool = new Pool(5);
			pool.define('greet', `${__dirname}/sample-module`);
			return P.join(pool.greet.apply('World!!'), pool.greet.apply('Earth!!')).spread(function (res1, res2) {
	...
	describe('Queue behavior', function () {
		it('should process multiple jobs sequentially for chunksizes of 1', function () {
			...
			return P.all([
				pool.map(_.range(501), fn, 1).then(function () {
					jobsCompleted++;
					jobsCompleted.should.equal(1);
				}),
				pool.map(_.range(502), fn, 1).then(function () {
					jobsCompleted++;
					jobsCompleted.should.equal(2);
				}),
				pool.map(_.range(503), fn, 1).then(function () {
					jobsCompleted++;
		...
		it.skip('should utilize all workers at once', function () {
			...
			return P.all([
				pool.map(_.range(1000), fn, 500).then(function () {
					jobsCompleted++;
					jobsCompleted.should.equal(2);
				}),
				pool.map(_.range(1), fn, 1).then(function () {
					jobsCompleted++;
					jobsCompleted.should.equal(1);
				}),
			]);
